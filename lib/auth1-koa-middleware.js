...
module.exports = ({
  ...
  const getTokenFromSession = (ctx) => {
    if (!ctx.session[sessionNamespace]) {
      throwError('User not logged')
    }
    auth1.setOauth2Tokens(ctx.session[sessionNamespace])
  }
  ...
  const setHtmlResponse = (ctx, params) => {
    ...
    if (resultParams.accessToken) {
      ...
      resultParams.errorCode = resultParams.errorCode || 'unknown'
    }
  ...
  const authorize = async (ctx) => {
    try {
      ...
      ctx.session[sessionNamespace] = await auth1.getToken(getParams(ctx, {
        code: code
      }))
      ...
      return setHtmlResponse(ctx, {
        accessToken: ctx.session[sessionNamespace].access_token,
        expiresIn: ctx.session[sessionNamespace].expires_in
      })
    } catch (err) {
      handleError(ctx, err, true)
    }
  ...
  const refresh = async (ctx) => {
    try {
      ...
      ctx.session[sessionNamespace] = await auth1.refreshToken(getParams(ctx))
      ctx.status = 200
      ctx.body = {
        access_token: ctx.session[sessionNamespace].access_token,
        expires_in: ctx.session[sessionNamespace].expires_in
      }
    } catch (err) {
      handleError(ctx, err)
    }
  ...
  const logout = async (ctx) => {
    try {
      ...
      ctx.session[sessionNamespace] = undefined
      ctx.status = 204
    } catch (err) {
      handleError(ctx, err)
    }
