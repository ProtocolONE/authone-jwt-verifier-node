...
class JwtVerifier {
  constructor (options, storage) {
    ...
    this.config = {}
    const requiredOptions = ['issuer', 'clientId', 'clientSecret']
    requiredOptions.forEach(key => {
      const value = options[key]
      if (!value) {
        throw new Error(`Required option '${key}' not set or empty`)
      }
      this.config[key] = value
    })
    if (typeof (options.redirectUrl) === 'undefined') {
      throw new Error(`Required option 'redirectUrl' not set`)
    }
    this.config.redirectUrl = options.redirectUrl
    this.config.scopes = options.scopes || []

    this.config.endpoint = {
      authUrl: this.config.issuer + '/oauth2/auth',
      tokenUrl: this.config.issuer + '/oauth2/token',
      userInfoUrl: this.config.issuer + '/oauth2/userinfo',
