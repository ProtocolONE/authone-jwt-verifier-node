...
class Auth1Client {
  ...
  async request (url, params) {
    ...
    if (options.useAuth) {
      if (options.authorizationMethod === 'header') {
        ...
        this.log('debug', 'Using header authentication. Authorization header set to %s', basicHeader)

        options.headers.Authorization = `Basic ${basicHeader}`
      } else {
        this.log('debug', 'Using body authentication')
    ...
    if (options.method !== 'GET') {
      ...
      this.log('debug', 'Bypass body due to GET request')
    }

    this.log('debug', 'Creating request to: (%s) %s', options.method, options.url)
    this.log('debug', 'Using options: %j', options)
  ...
  async setOauth2Tokens (tokenData, params) {
